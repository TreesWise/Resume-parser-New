...
# print(f"APP_HOME is set to: {APP_HOME}")
...
# Function to update the mapping dictionary
...
#     # Initialize the dictionary (load existing one if needed)
#     mapping_dict = {}

#     try:
#         # Read the current dict_file.py and load it into mapping_dict
#         with open('dict_file.py', 'r', encoding='utf-8') as f:
#             content = f.read()
...
#     # Loop through the fetched rows and add new mappings if they don't exist
#     for row in rows:
...
#         value = value.replace("\n", "\\n")
#         # Escape any internal double quotes inside the string
#         value = value.replace('"', '\\"')
#         return value

#     # Only update dict_file.py with new or modified entries
#     try:
#         with open('dict_file.py', 'w', encoding='utf-8') as f:
#             f.write("mapping_dict = {\n")
#             # Write existing and new mappings to the file
#             for key, value in mapping_dict.items():
#                 # Escape key and value
#                 key = escape_value(key)
#                 value = escape_value(value)

#                 # Write the key-value pair on a single line
#                 f.write(f'    "{key}": "{value}",\n')
#             f.write("}\n")

#         print("[INFO] dict_file.py updated successfully.")
#     except Exception as e:
#         print(f"[ERROR] Failed to update dict_file.py: {e}", flush=True)
